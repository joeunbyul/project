...
public class ModifyWrite extends Activity implements View.OnClickListener {
    ...
    /*글 등록*/
    ImageButton register_write,regist_undo;

    /*오브젝트*/
    private MypageNeedObject mypageNeedObject;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.modify_write);

        if(mypageNeedObject==null){
            mypageNeedObject = new MypageNeedObject();
        }

        //물품명
        edit_register_write = (EditText)findViewById(R.id.edit_modify_write);
        //설명글
        edit_write_info = (EditText)findViewById(R.id.edit_modify_info);

        Intent i = getIntent();
        category = i.getExtras().getString("category");
        location = i.getExtras().getString("location");
        need_id = i.getExtras().getInt("need_id");
        edit_register_write.setText(i.getExtras().getString("itemname"));
        edit_write_info.setText(i.getExtras().getString("content"));
        ...
        regist_undo = (ImageButton)findViewById(R.id.delete_register_write);
        regist_undo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                Intent intent = new Intent(ModifyWrite.this, MypageNeed.class);
    ...
    public void regist_do(View v){
        ...
        mypageNeedObject.mypage_need_title = edit_register_write.getText().toString();
        mypageNeedObject.mypage_need_category = category;
        mypageNeedObject.mypage_need_contents = edit_write_info.getText().toString();
        mypageNeedObject.mypage_need_location = mapsubtitle.getText().toString();

        if(mypageNeedObject.mypage_need_title== null || mypageNeedObject.mypage_need_title.length() <=0){
            Toast.makeText(MyApplication.getmContext(),"물품명을 입력하세요.",Toast.LENGTH_LONG).show();
        }else if(mypageNeedObject.mypage_need_category == null || mypageNeedObject.mypage_need_category.length() <=0){
            Toast.makeText(MyApplication.getmContext(),"카테고리를 선택하세요.",Toast.LENGTH_LONG).show();
        }else if(mypageNeedObject.mypage_need_contents == null || mypageNeedObject.mypage_need_contents.length() <=0) {
            Toast.makeText(MyApplication.getmContext(), "설명글을 입력하세요.", Toast.LENGTH_LONG).show();
        }else{
            new AsyncMypageNeedModify().execute(mypageNeedObject);
        }
    ...
    public void onClick(View v){
        if(v.getId() == R.id.write_selectmap) {
            String item_name = edit_register_write.getText().toString();
            String info = edit_write_info.getText().toString();
            writeinfo_editor.putString("write_title",item_name);
            writeinfo_editor.putString("write_contents",info);
            writeinfo_editor.apply();

            Intent intent = new Intent(ModifyWrite.this, ModifyWriteMapActivity.class);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        /*프리퍼런스 가져오기*/
                ...
                String subtitle_data = data.getStringExtra("subtitle");
    ...
    public class AsyncMypageNeedModify extends AsyncTask<MypageNeedObject, Integer,String>{
        @Override
        ...
        @Override
        protected String doInBackground(MypageNeedObject... wantItmeInfo) {
            boolean flag;
            String wantItemInsertResult = "";
            MypageNeedObject reqParams = wantItmeInfo[0];
           response = null;
            ...
            try {
                ...
                RequestBody postBody = new FormBody.Builder()
                        .add("title",reqParams.mypage_need_title)
                        .add("category",reqParams.mypage_need_category)
                        .add("article",reqParams.mypage_need_contents)
                        .add("location",reqParams.mypage_need_location)
                        .build();

                final Request request = new Request.Builder()
                        .url(NetworkDefineConstant.SERVER_URL_NADO_WANTITEM_INSERT+"/"+need_id)
                        .post(postBody)
                        .build();
        ...
        protected void onPostExecute(String result) {
            ...
            if(result != null){
                ...
                if (result.equalsIgnoreCase("put success")){
                    ...
                    Intent main = new Intent(MyApplication.getmContext(),MypageNeed.class);
