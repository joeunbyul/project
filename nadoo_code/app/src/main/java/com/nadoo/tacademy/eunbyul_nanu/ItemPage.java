...
public class ItemPage extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        item_id = item.getExtras().getInt("item_id");
        item_photo = item.getExtras().getString("item_photo");
        item1 = item.getExtras().getString("item1");
        item2 = item.getExtras().getString("item2");
        item3 = item.getExtras().getString("item3");
        ...
        item_userid = item.getExtras().getInt("item_userid");
        ...
        Glide.with(MyApplication.getmContext()).load(item1).into(itemPage_photo);
        ...
        item_page_chatting.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String chatname="";
                if (nick.toString().equals("") || item_nickname.equals(""))
                    return;

                if(nick.toString().compareTo(item_nickname)<0){
                    chatname = nick.toString() + item_nickname;
                }else if(nick.toString().compareTo(item_nickname)>0){
                    chatname = item_nickname+nick.toString();
                }
                Intent chat = new Intent(MyApplication.getmContext(), ChatActivity.class);
                chat.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                chat.putExtra("chatName", chatname);
                chat.putExtra("userName", nick.toString());
        ...
        LinearLayout item_page_profile = (LinearLayout)findViewById(R.id.item_page_profile);
        item_page_profile.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                prefs = getSharedPreferences("user_pref", MODE_PRIVATE);
                String nick = prefs.getString("nickname","");
                if(!nick.equals(item_nickname)) {
                    ...
                    profile.putExtra("user_img",item_userphoto);
                    profile.putExtra("user_name", item_nickname);
                    profile.putExtra("user_id",item_userid);
        ...
        itemPage_price.setText(item_price+"원");
